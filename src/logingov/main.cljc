(ns logingov.main
  "Kotodama WASM entrypoint for the logingov clean-room actor (L4) — Clojure port.
  ...
  facts (`logingov.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "logingov")
(def tier "L4")
...
(def entity-specs
  [{:entity "Filing"      :plural "filings"      :id-prefix "logingov_fil"
    :fields [:filerId :form :period :status]    :required [:form :period]
    :coerce {}                       :refs {}}
   {:entity "Entity"      :plural "entities"     :id-prefix "logingov_ent"
    :fields [:name :ein :jurisdiction]          :required [:name :ein]
    :coerce {}                       :refs {}}
   {:entity "Dataset"     :plural "datasets"     :id-prefix "logingov_dat"
    :fields [:name :agency :updatedAt]          :required [:name :agency]
    :coerce {}                       :refs {}}
   {:entity "Application" :plural "applications" :id-prefix "logingov_app"
    :fields [:applicantId :program :status]     :required [:program :status]
    :coerce {}                       :refs {}}
   {:entity "PublicRecord" :plural "publicrecords" :id-prefix "logingov_pub"
    :fields [:recordType :jurisdiction :recordedAt] :required [:recordType :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Notice"      :plural "notices"      :id-prefix "logingov_not"
...
(defn emit-facts
  "EAVT facts for one record: {\"logingov.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "logingov-compat" :tier tier :entities entities} 200])
...
(defn start! [] :logingov-compat/ready)
